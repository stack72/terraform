			...
			"frontend_ip_configuration": &schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"load_balancer_rules": &schema.Schema{
							Type:     schema.TypeSet,
							Computed: true,
			...
			"backend_address_pool": &schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"backend_ip_configurations": &schema.Schema{
							Type:     schema.TypeSet,
							Computed: true,
							Elem:     &schema.Schema{Type: schema.TypeString},
							Set:      schema.HashString,
			...
			"load_balancing_rule": &schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"frontend_ip_configuration_id": &schema.Schema{
							Type:     schema.TypeString,
							Optional: true,
							Computed: true,
						},

						"backend_address_pool_id": &schema.Schema{
							Type:     schema.TypeString,
							Optional: true,
							Computed: true,
						},
			...
			"probe": &schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"load_balance_rules": &schema.Schema{
							Type:     schema.TypeSet,
							Computed: true,
							Elem:     &schema.Schema{Type: schema.TypeString},
							Set:      schema.HashString,
			...
			"inbound_nat_rule": &schema.Schema{
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"backend_ip_configuration_id": &schema.Schema{
							Type:     schema.TypeString,
							Computed: true,
						},
					},
...
func resourceArmLoadbalancerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := loadBalancerClient.CreateOrUpdate(resGroup, name, loadbalancer)
	if err != nil {
		return err
	}
...
func resourceArmLoadbalancerDelete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = loadBalancerClient.Delete(resGroup, name)
...
func expandAzureRmLoadbalancerBalancingRules(d *schema.ResourceData) ([]network.LoadBalancingRule, error) {
	...
	for _, configRaw := range configs {
		...
		if v, ok := data["idle_timeout_in_minutes"]; ok {
			timeout := v.(int)
			properties.IdleTimeoutInMinutes = &timeout
		}
